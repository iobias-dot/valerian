class StudentGuardiansController < ApplicationController
  ...
  def index
    @student_guardians = Student_Guardian.all
  end
  ...
  def new
    @student_guardian = Student_Guardian.new
  end
  ...
  def create
    @student_guardian = Student_Guardian.new(student_guardian_params)

    respond_to do |format|
      if @student_guardian.save
        format.html { redirect_to @student_guardian, notice: "StudentGuardian was successfully created." }
  ...
  def update
    respond_to do |format|
      if @student_guardian.update(student_guardian_params)
        format.html { redirect_to @student_guardian, notice: "StudentGuardian was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to student_guardians_path, notice: "StudentGuardian was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_student_guardian
      @student_guardian = Student_Guardian.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def student_guardian_params
      params.expect(student_guardian: [ :guardian_id, :student_id, :guardianship])
    end
end
